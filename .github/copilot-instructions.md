...
- For tool/skill/proposal routing, follow up with **`mcp-vertex_agent_catalog`**
  (default `mode: "compact"`; pass `section` or `query` to narrow). The full
  catalog is regenerated by `bun run catalog:generate` and lives at
  `docs/mcp-vertex/agent-catalog.generated.json`; per-host fragments live under
  [`docs/mcp-vertex/host-hints/`](../docs/mcp-vertex/host-hints/) (see
  [`copilot-instructions.generated.md`](../docs/mcp-vertex/host-hints/copilot-instructions.generated.md)
  for the Copilot-specific discovery block).
