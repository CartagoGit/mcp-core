...
- `docs/mcp-vertex/agent-catalog.generated.json` — checked-in snapshot of the
  unified discovery surface (tools + skills + proposals). Hosts that need a
  stable route to this surface read it directly; the per-host fragments under
  `docs/mcp-vertex/host-hints/` are generated from the same artifact by
  `bun run catalog:hints`.

## Discovery (generated)

The canonical first move for any new agent is

```text
mcp-vertex_overview { compact: true } -> mcp-vertex_agent_catalog
```

then route via the unified catalog (mode `compact` for orientation, mode `full`
or `section=<tools|skills|proposals>` for specifics, `query=...` to filter).
The human-edited instruction files keep only the stable narrative; the
discovery surface itself lives under [`docs/mcp-vertex/host-hints/`](docs/mcp-vertex/host-hints/)
and is regenerated by `bun run catalog:hints`. Drift between the live registry
and the checked-in artifact fails `bun run catalog:check` / `bun run catalog:hints:check`.
