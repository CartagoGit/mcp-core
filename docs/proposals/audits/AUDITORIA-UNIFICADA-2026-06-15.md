...
| N19 | **Plugins `docs` y `deps/security`** (autocontenidos, como `search`) | (orig.) | 🟡 **`docs` HECHO** (`@cartago-git/mcp-docs`, 9º paquete: `docs_list`/`docs_read`, navegación curada de markdown, agnóstico, anti-traversal, outputSchema + e2e). Pendiente `deps/security`. |
