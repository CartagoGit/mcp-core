...
| N16 | **`outputSchema` Zod por tool** (structuredContent ya está; falta declarar el schema → validación/UMI en clientes) | S | 🟡 **15/~30 HECHOS con red e2e real**: memory(4), search(1), core-meta(overview compact+full vía uniones, knowledge, get_validation_matrix), git(4), quality(2), notification(1). El SDK valida `structuredContent` sólo en éxito (`isError` exento, `server/mcp.js:193`); red `outputschema.e2e.spec.ts` ensambla los 7 plugins y llama cada tool read-only por el protocolo (git init real). **Pendiente**: proposals (15 tools, varios *action-multiplexed* agent_names/task_queue/agent_lock/delegate/round_context → requieren schema por-acción/permisivo + e2e por variante) y bootstrap/scaffold (dev-time). |
