...
# a025 — Auditoría independiente y exhaustiva — Claude Code (Sonnet 4.6)
...
- **Regular**: 3 `outputSchema` `z.object({}).catchall(z.unknown())` en `bootstrap-tool.ts` y 1 en `scaffold-tool.ts` siguen presentes — ya cubiertos por `l125`/`l122` (no se duplica).
...
- **Perfecto**: `agent-loop-detector.ts` + `loop-detector-service.ts` (wiring confirmado en `plugins/proposals/src/index.ts:108` y verificado en vivo durante `bun run test`, logs `loop-detector: agent "a1" is stuck...`) — el hallazgo de soft-loop de `a021` (`H4`/`f120`) está **resuelto**, no diferido.
...
- **Regular (ya rastreado)**: race condition en `sync-proposal-registry.ts:331` (`writeFile` sin `writeFileAtomic` antes del `rename`) — sigue abierta, rastreada en `f122` (`status: ready`).
- **Regular (ya rastreado)**: I/O síncrono en `notification/watcher.ts` (11 usos de `existsSync`/`readdirSync`/`readFileSync` dentro de `setInterval`) y en `rules/frameworks/manifest.ts` (3 usos de `writeFileSync`/`readFileSync`) — siguen abiertas, rastreadas en `f123` y `l125` respectivamente.
...
- **Nota de proceso**: el paquete está deliberadamente incompleto (proposal `f125` en `status: ready`, slices S1–S11 pendientes); el bug de tipos es responsabilidad de ese trabajo en curso, no un defecto de diseño del shell `IHostAdapter`.

#### VS Code Extension (`apps/vscode`)
- **Bien**: i18n completo (12 idiomas × 8 keys, verificado con `check:i18n`). Sigue excluido de `vitest.config.ts` raíz como proyecto propio (el slot fue ocupado por `apps/ide`, no por `apps/vscode`) — issue de cobertura de gate ya señalado por `a021`/`a023`, sin novedad aquí.

### 2. Eficiencia / tokens / bucles / bloqueos
- **Bucles**: el detector de loops (`l103`) está en producción y se disparó en vivo durante los tests (`no-progress` y `exact-repeat`), confirmando que la mitigación de `a021` funciona end-to-end, no solo a nivel de unidad.
...
- Resolver `f122`, `f123` o `l125` (ya creadas por auditorías previas; se referencian, no se duplican).
...
- Completar la propuesta `f125` (IDE extension v2) — solo se corrigen los bugs de tipos que bloqueaban el gate global.
- Re-auditar a fondo `apps/vscode` (ya cubierto por `a021`/`a023`).
...
- **Files**: docs/proposals/ready/a025-21-06-2026-claude-code-sonnet-4-6-repositorio.md
...
| Apps | `ls apps` | 3 (`ide` — nuevo, en curso vía `f125`; `vscode`; `web`) |
...
| Catchalls residuales | `grep -rc 'catchall(z.unknown())' packages/core/src plugins/*/src` | 6 (sin cambio; cubiertos por `l125`/`l122`) |
| I/O síncrono en `notification/watcher.ts` | `grep -c 'existsSync\|readdirSync\|readFileSync' plugins/notification/src/lib/watcher.ts` | 11 (sin cambio; cubierto por `f123`) |
| I/O síncrono en `rules/frameworks/manifest.ts` | `grep -c 'writeFileSync\|readFileSync' plugins/rules/src/lib/frameworks/manifest.ts` | 3 (sin cambio; cubierto por `l125`) |
| Race condition `sync-proposal-registry.ts:331` | `grep -n 'await writeFile(sourcePath' plugins/proposals/src/lib/proposals/sync-proposal-registry.ts` | presente, sin `writeFileAtomic` (sin cambio; cubierto por `f122`) |
...
| H4 | P1 | `Base.astro` seguía sin `data-pagefind-body` (hallazgo `H3` de `a021`, asignado a un slice que nunca se ejecutó); Pagefind indexaba 500 páginas con ruido de navegación en vez de 338 páginas de contenido. | [Base.astro](file:///home/cartago/_projects/mcp-vertex/apps/web/src/layouts/Base.astro) | Resolved in slice `S2` |
...
| H6 | P3 | `plugins/audit/package.json#files` omite `LICENSE` (y el archivo ni siquiera existe en el directorio), a diferencia de los otros 12 plugins publicables. Hallazgo `H6` de `a023`, seguía abierto. | [package.json](file:///home/cartago/_projects/mcp-vertex/plugins/audit/package.json) | Resolved in slice `S6` |
| H7 | P1 | Race condition: `writeFile` sin `writeFileAtomic` antes del `rename` a `historical/` en `sync-proposal-registry.ts:331`. Ventana de inconsistencia ante un `kill -9`. | [sync-proposal-registry.ts:331](file:///home/cartago/_projects/mcp-vertex/plugins/proposals/src/lib/proposals/sync-proposal-registry.ts) | Deferred to Proposal `f122` (ya existente, `status: ready`) |
| H8 | P1 | I/O síncrono (`existsSync`/`readdirSync`/`readFileSync`) en `notification/watcher.ts` dentro de un `setInterval`, bloqueando el event loop. | [watcher.ts](file:///home/cartago/_projects/mcp-vertex/plugins/notification/src/lib/watcher.ts) | Deferred to Proposal `f123` (ya existente, `status: ready`) |
| H9 | P2 | I/O síncrono (`writeFileSync`/`readFileSync`) en `rules/frameworks/manifest.ts` y 6 `outputSchema` catchall residuales en `bootstrap-tool.ts`/`scaffold-tool.ts`/`rules-tools.ts`/`adopt.tool.ts`. | [manifest.ts](file:///home/cartago/_projects/mcp-vertex/plugins/rules/src/lib/frameworks/manifest.ts) | Deferred to Proposal `l125` (ya existente, `status: ready`) |
...
| Plugins | 8.7 | 13 plugins sanos en general; 3 hallazgos de I/O/race ya rastreados (`f122`/`f123`/`l125`) siguen sin cerrar tras 2 auditorías previas. |
...
| **Total (Average)** | **8.5** | **Base sólida; el principal riesgo no es de diseño sino de "gate verde" momentáneo durante trabajo concurrente — mitigado en esta auditoría, pero recurrente mientras `f122`/`f123`/`l125` sigan abiertas.** |

## Notes

- Esta auditoría es independiente de `a021` (Antigravity/Gemini 3.5 Flash), `a022` (Copilot MiniMax-M3) y `a023` (Copilot GPT-5.4), todas fechadas 2026-06-21. No reescribe ninguna; valida el estado *actual* del código y confirma que `f122`, `f123` y `l125` —ya creadas por auditorías anteriores— siguen abiertas y cubren correctamente los hallazgos de I/O síncrono y race conditions sin necesidad de nuevas propuestas duplicadas.
- El hallazgo de soft-loop de `a021` (`H4`, antes asignado a una hipotética `f120`) está **confirmado resuelto** en producción (`agent-loop-detector.ts` + `loop-detector-service.ts`, wiring en `plugins/proposals/src/index.ts:108`, disparado en vivo durante `bun run test`).
