...
# f101 — Header persistente con transiciones + surface completo en la web

> **Estado: TODO para revisión.** Continúa f100 con tres frentes: (1)
...
Después de f100 la web ya tiene estructura de páginas (home minimalista +
...
- Internacionalizar los tutoriales (eso queda en f100 slice s7).
...
- Componentes: `PromptsSection.astro`, `ResourcesSection.astro`,
  ...
  plugin.
- `gen-section-pages.sh` (existente, creado en f100) se amplía con 3
  secciones más.
...
- **id: s3** — Páginas `/prompts`, `/resources`, `/knowledge` + componentes.
  ...
  - Reusar `gen-section-pages.sh` de f100. Añadir las 3 secciones al loop.
...
- [ ] `/prompts`, `/resources`, `/knowledge` renderizan datos reales en
      12 idiomas, con descripciones localizadas (post f100 s4, en este
...
| 12 × 9 keys i18n nuevas = 108 traducciones a mano | Slice s3 arranca con `en` completo y deja los otros 11 idiomas con `en` como fallback en el dict. Las traducciones se añaden en una propuesta posterior (f102) usando un script de bootstrap (DeepL opcional). |
