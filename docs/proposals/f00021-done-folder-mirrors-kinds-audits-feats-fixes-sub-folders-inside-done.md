...
│   ├── feats/              # kind: feat   (~10-15 files: f00004..f00001, l99..l113 re-tagged)
...
- acceptance:
  ...
  - "Filename prefix is updated to match the real kind (e.g. `l99-feat-multi-model-audit-plugin.md` → `f00004-feat-multi-model-audit-plugin.md` if its kind is feat; or stays `l99` if its real kind is legacy)."
  - "`PROPOSAL_KIND_BY_PREFIX` lookup is updated/checked for every rename; `bun run lint:proposals` passes."
  - "The `id:` frontmatter field stays the same as the original (e.g. `id: l99` stays `l99` even if the filename becomes `f00004-...`) so cross-proposal references keep working."
- status: pending
...
- gate: lint
- acceptance:
  - "f00016-f00018 live under `docs/proposals/done/feats/`."
  - "A `docs/proposals/done/README.md` documents the convention (one sub-folder per `kind`; only `audits/`, `feats/`, `fixes/` for now; root of `done/` is reserved for the legacy `n001-SESION-*.md` and `AUDITORIA-UNIFICADA-*.md` style summaries that predate the convention)."
  - "Cross-references (`related: f00016`, `related: l99`, etc.) still resolve because `id:` frontmatter is preserved across renames."
- status: pending
