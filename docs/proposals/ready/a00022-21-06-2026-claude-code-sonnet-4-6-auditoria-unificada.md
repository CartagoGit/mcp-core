...
- Resolver `f00020`, `f00019`, `l00003`, `l00007` o `l00008` — se referencian como ya-cubiertos, no se reimplementan ni se tocan sus archivos.
- Resolver la duplicación documentada entre `l00007` y `l00008` (mismos dos archivos de catchall, `rules-tools.ts:199` y `adopt.tool.ts:81`) — se documenta como meta-hallazgo informativo (§ Findings, fila "META"), no se decide aquí cuál propuesta debe ceder el slice.
...
| Findings deduplicados referenciados a propuesta `ready/` existente | f00020, f00019, l00007, l00008 | 6 (con 1 meta-hallazgo de duplicación l00007↔l00008 documentado) |
...
- **Familia D** (medición longitudinal, el prerequisite de A y B) — H1, **ya cubierta por `l00003`** (`status: ready`), no duplicada por `a00025`.
...
- **`a024a` (cache de respuestas MCP)** — **diferida explícitamente**, prerequisite no cumplido (`l00003` aún `status: ready`, no `done`). No se crea en esta sesión.
- **`a024b` (snapshot plugin)** — **diferida explícitamente**, doble prerequisite no cumplido (`l00003` merged AND medición que confirme `git diff` en el top 3 de bytes). No se crea en esta sesión.
...
- **`a024d`** — no es una propuesta independiente real (el propio `a00025` la describe como "copy en `TOKEN-BUDGETS.md` referenciando `l00003`", sin prerequisite propio); se trata como nota de documentación pendiente, no como propuesta a crear.
...
- El meta-hallazgo META-1 (duplicación `l00007`↔`l00008`) y la nota H-I18N-DUP quedan deliberadamente sin propuesta de seguimiento — son follow-ups de coordinación entre propuestas existentes, no hallazgos nuevos de código, y exceden el mandato de esta consolidación.
