...
reservedFiles:
    ...
    - docs/proposals/RESUMEN-SESION-2026-06-17.md
...
related:
    - f113 # proposal state machine — defines the 7-status DFA + the 12 kinds; f126 must not break it
    - f119 # done-folder mirrors kinds — IN PROGRESS; f119 is renumbering audits a1..a20 inside done/audits/ today
    - f125 # IDE extension v2 — does not touch IDs but has reservedFiles that include docs/proposals/done/feats/
---

# f126 — Renumerar proposals con padding de 5 dígitos, por fecha de creación, dentro de cada familia
...
- `f`: 122, 123, 125 (3 fixes; huecos en 124 — que sí existe como `l124` no; sí como `x124`; y en 100-121 no usados)
- `l`: 114, 115, 116, 117, 118, 119, 120, 121, 122, 125, 126, 127 (12 legacy; huecos en 123, 124)
- `x`: 123, 124 (2 done)

Esto causa tres problemas:

1. **Los números no reflejan el orden cronológico real.** `a1` y `a2` (los audits del 14/06 y 15/06 — los más antiguos) ya están en `done/audits/` renombrados por `f119`; pero el resto del catálogo empieza en `a21`, dejando huecos inexplicables entre `a4` y `a21`.
2. **Los IDs son ambiguos en logs y PRs.** `f122` y `l122` coexisten; `f123` y `x123` también. Un grep `grep -E '\b1[0-9]{2}\b'` en un log devuelve los dos.
3. **No hay un ancho fijo**, así que cuando `f*` llegue a 1000 o `a*` llegue a 100, los IDs tendrán longitudes distintas y romperán el layout de tablas, columnas de dashboard, y orden lexicográfico (`f122` < `f23` en ASCII aunque `f23` sea más antiguo).

**Esta propuesta** arregla los tres problemas con un único cambio:

- **Cada familia** (`a`, `c`, `f`, `l`, `p`, `x`) mantiene su namespace propio (no se mezclan — invariante del workflow f113 §3.1).
...
- [ ] `docs/proposals/index.json` lista los 22 IDs existentes (más los 20 audits que f119 ya reubicó en `done/audits/`) con IDs padded: `a00001..a00020` (audits) + `a00021..a00024` (audits nuevos del 21/06), `f00001..f00003` (f121..f123 + el hueco), `l00001..l00012` (l114..l127), `x00001..x00002` (x123, x124). Si al ejecutar S1 aparecen más IDs en `done/feats/` o `done/fixes/` que el index actual no conoce, se incluyen también.
...
- [ ] Las dos menciones explícitas en `f119` ("20 audit files renumbered a1..a20") y el `RESUMEN-SESION-2026-06-17.md` quedan actualizadas al nuevo padded (`a00001..a00020`).
...
3. **Refs internas** en otros proposals (`related: [f113, f119, f125]`, links `[f113](done/f113-...)`, menciones en prosa como "After landing f113..."). Solo las que apunten a un ID cuyo número haya cambiado.
...
7. **`docs/proposals/RESUMEN-SESION-2026-06-17.md`** — las menciones a IDs viejos se actualizan al nuevo padded.

## Out of scope (lo que NO toca)

- El modelo DFA de 7-status (f113) — intacto.
...
- Las propuestas en `done/audits/` que ya renombró `f119` (a1..a20) — se les reescribe el `id:` para que sea `a00001..a00020` pero NO se mueven de carpeta.
...
|---------|-------------------------|---------------------------------------------------------------------------------------|
| `a`     | 21, 22, 23, 24          | `a00021`, `a00022`, `a00023`, `a00024` (los 4 del 21/06 — los 20 de `done/audits/` ya fueron `a1..a20` por f119 y se renumeran a `a00001..a00020`) |
| `f`     | 122, 123, 125 (+ los renombrados por f119 en done/) | `f00001`, `f00002`, `f00003` (los más recientes del catálogo ready/in-progress)        |
...
- **s3 — Actualizar referencias externas (3 archivos, sin solapar con s2)**
  - files: [`docs/proposals/RESUMEN-SESION-2026-06-17.md`, `docs/proposals/in-progress/f119-*.md`, `scripts/lint-proposals.ts`]
  - agent: `implementation_runner`
  - gate: `lint`
  - acceptance:
...
- **f119 está `in_progress` y renombra `a1..a20` en `done/audits/`**. Si f119 cierra antes que s2, el mapa de s1 ya incluye esos IDs como `a00001..a00020`. Si f119 cierra después, s1 debe coordinarse con f119 para no renombrar dos veces — la política es **s1 espera a f119** si f119 no ha hecho `git mv` aún. **Recomendación**: bloquear el lock de f119 antes de empezar s1, ejecutar s1, liberar el lock.
- **f125 (IDE extension) tiene `reservedFiles: [..., docs/proposals/done/feats/]`.** Eso podría colisionar con los archivos que f126 va a renombrar en `done/feats/`. Resolución: s2 lee el `reservedFiles` de f125 antes de hacer `git mv` y aborta si encuentra conflicto (en la práctica, f125 aún no está implementado, así que el riesgo es bajo — pero s2 lo verifica).
...
```bash
git revert <merge-commit-de-f126>
```
