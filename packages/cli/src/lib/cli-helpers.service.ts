/**
 * cli-helpers.ts — single source of truth for the small helpers every
 * CLI surface reuses (groups/*.ts, registry.ts, init/*.ts).
 *
 * Extracted in f00046 follow-up / a00036 follow-up so `data`, `scalarArg`,
 * `hasFlag`, `request`, and the type guard `isRecord` stop being
 * copy-pasted across `git.ts` and `registry.ts`. The two thin
