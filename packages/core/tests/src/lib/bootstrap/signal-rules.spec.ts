...
describe('DEFAULT_SIGNAL_RULES (declarative table)', () => {
	it('lists the ten built-in signal rules', () => {
...
describe('matchSignals', () => {
	it('emits the no-manifest signal when no package.json + unknown language', () => {
	...
	it('emits the mcp-server-missing signal for a fresh project', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({
				hasPackageJson: true,
				language: 'typescript',
	...
	it('emits the mcp-server-exists signal when the project already has one', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({
				hasPackageJson: true,
				language: 'typescript',
	...
	it('formats the framework signal with the framework name', () => {
		const ctx: ISignalContext = {
	...
	it('formats the monorepo-tool signal with the tool name', () => {
		const ctx: ISignalContext = {
	...
	it('emits the non-js-stack signal for a Python project', () => {
		const ctx: ISignalContext = {
	...
	it('does NOT emit the non-js-stack signal for TypeScript', () => {
		const ctx: ISignalContext = {
	...
	it('emits the agent-configs signal listing the configs', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({
	...
	it('emits the custom-extra-tools signal when the host-config has custom bits', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({}),
	...
	it('emits the custom-vertex-config signal when mcp-vertex.config.json has config', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({}),
			hasCustomExtraTools: false,
	...
	it('emits the ci signal listing the CI systems', () => {
		const ctx: ISignalContext = {
	...
	it('emits signals in priority order (manifest → mcp → framework → …)', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({
				hasPackageJson: true,
				framework: 'react',
	...
	it('a custom rule table adds signals without forking the matcher', () => {
		const ctx: ISignalContext = {
			analysis: makeAnalysis({}),
			hasCustomExtraTools: false,
			hasCustomVertexConfig: false,
...
describe('integration: analyzeProject uses the rule table', () => {
	it('emits the same signals as the pre-refactor chain (regression guard)', () => {
