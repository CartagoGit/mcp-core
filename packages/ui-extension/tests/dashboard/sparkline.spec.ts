...
describe('sparklinePath', async () => {
	it('returns empty string for empty values', async () => {
		expect(sparklinePath([], 100, 30)).toBe('');
	});

	it('returns a flat line for a single value', async () => {
		const d = sparklinePath([5], 100, 30);
		expect(d).toContain('M 0');
		expect(d).toContain('L 100');
	});

	it('returns M then L segments for multiple values', async () => {
	...
	it('handles a constant series by collapsing to the midline', async () => {
	...
	it('clamps to height when range = 0 (all values equal)', async () => {
	...
	it('returns empty for non-positive width or height', async () => {
		expect(sparklinePath([1, 2, 3], 0, 30)).toBe('');
		expect(sparklinePath([1, 2, 3], 60, 0)).toBe('');
	});

	it('inverts the y axis (max at top, min at bottom)', async () => {
