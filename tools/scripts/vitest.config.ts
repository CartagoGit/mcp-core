 ...
 * (`tools/scripts/lib/silence-console-setup.ts`); importing it would be
 * circular, so we wire the absolute path via `sharedSetupFiles` from
 * the shared root — vitest resolves it before any spec is collected.
 */
